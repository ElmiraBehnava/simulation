...
def arrival(future_event_list, state, clock, data, customer, customer_type):
    ...
    if state["First Server Status"] == 0 and customer_type == "Normal":
    ...
    elif (state["First Server Status"] == 1 and customer_type == "Normal") or (
    ...
    next_customer_type = random.choices(["Normal", "VIP"], [70, 30])[0]
    fel_maker(
        future_event_list,
        "Arrival",
        clock,
